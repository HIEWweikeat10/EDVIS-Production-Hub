...
// POST /api/delete
// Body: { code, adminKey, confirmText }
// Requires correct admin key AND confirmText === "DELETE PROJECT".
// All child rows cascade automatically via ON DELETE CASCADE.
const { getEnv, dbGet, dbDelete, json, err, sha256 } = require('./_db')
...
module.exports = async function handler(req, res) {
  ...
  try {
    const env = getEnv()
    const { code, adminKey, confirmText } = req.body || {}
    if (!code)     return err(res, 'code is required', 400)
    if (!adminKey) return err(res, 'adminKey is required', 400)
    if (String(confirmText || '').trim().toUpperCase() !== 'DELETE PROJECT')
      return err(res, 'CONFIRM_REQUIRED: type DELETE PROJECT to confirm', 400)
    ...
    await dbDelete('projects', `id=eq.${project.id}`, env)
    json(res, { ok: true, deleted: project.title })
  } catch (e) {
    err(res, e.message)
  }
}
