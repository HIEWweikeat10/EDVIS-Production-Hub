...
module.exports = function handler(req, res) {
  ...
  // Piggybacks on this endpoint (rather than a dedicated one) to stay under
  ...
  res.status(200).end(JSON.stringify({ ok: true, url: url.replace(/\/+$/, ''), anonKey, vapidPublicKey: vapidPublicKey || null }))
}
